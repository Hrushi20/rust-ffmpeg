...
impl Output {
    ...
    pub fn description(&self) -> String {
        unsafe {
            let long_name_length =
                avformat_wasmedge::avIOFormat_long_name_length(self.ptr(), 1) as usize;
            let long_name = vec![0u8; long_name_length];
            avformat_wasmedge::avOutputFormat_long_name(
                self.ptr(),
                long_name.as_ptr(),
                long_name_length,
            );
    ...
    pub fn extensions(&self) -> Vec<String> {
        unsafe {
            let extensions_length =
                avformat_wasmedge::avIOFormat_extensions_length(self.ptr(), 1) as usize;

            if extensions_length == 0 {
                Vec::new()
            } else {
                let extensions = vec![0u8; extensions_length];
                avformat_wasmedge::avOutputFormat_extensions(
                    self.ptr(),
                    extensions.as_ptr(),
                    extensions_length,
                );
    ...
    pub fn mime_types(&self) -> Vec<String> {
        unsafe {
            let mime_type_length =
                avformat_wasmedge::avIOFormat_mime_type_length(self.ptr(), 1) as usize;

            if mime_type_length == 0 {
                Vec::new()
            } else {
                let mime_type = vec![0u8; mime_type_length];
                avformat_wasmedge::avOutputFormat_mime_type(
                    self.ptr(),
                    mime_type.as_ptr(),
                    mime_type_length,
                );
    ...
    pub fn codec<P: AsRef<Path>>(&self, path: &P, kind: media::Type) -> codec::Id {
        ...
        unsafe {
            let short_name = "";
            let mime = "";
            let id = avformat_wasmedge::av_guess_codec(
                self.ptr(),
                short_name.as_ptr(),
                short_name.len(),
                path.as_ptr(),
